...
import 'package:AgentServe/domain/entities/booking.dart';
import 'package:AgentServe/domain/entities/provider.dart' as domain;
import 'package:AgentServe/domain/entities/service_request.dart';
import 'package:AgentServe/domain/entities/workflow_log.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:uuid/uuid.dart';
import 'package:AgentServe/data/notifications/notification_service.dart';

final agentServePlatformProvider = Provider<AgentServePlatformService?>(
  (ref) => null,
);
...
class WorkflowOrchestrator extends Notifier<WorkflowState> {
  ...
  Future<void> processUserRequest(String input) async {
    final _agentService = ref.read(agentServePlatformProvider);
    ...
    if (providers.isEmpty) {
      ...
      _addLog('AgentServe | Maps API', detail);
      _addLog(
        'AgentServe | Decision',
    ...
    _addLog(
      'AgentServe | Multi-step Reasoning',
  ...
  Future<void> bookProvider(domain.Provider provider) async {
    ...
    _addLog('AgentServe | Action Execution', 'Booking ${provider.name}...');
    ...
    triggerNotification(
      NotificationPayload(
        title: 'AgentServe',
        body:
            'Hi! Your booking with ${provider.name} for ${request.serviceType} is confirmed for ${request.time}. Ref ID: ${booking.id}. - AgentServe',
        type: 'whatsapp',
